...
docker run --name proxy-server-yannick -v /home/$USER/proxy-server-yannick/siteava:/etc/nginx/sites-available -d --network yannick-net -p 80:80 --restart always nginx
